...
app down. Import and verification reject unsupported lengths, and every app-side
conversion to `security::Seed` goes through a checked helper. Temporary entropy
used by that helper is zeroized after conversion.
...
- **`os/gui-server`**: the `gui-app` template, including `ShowModal` for the
  system QR scanner
- **read-only theme and touch-debug `os/settings`**
- **no `os/security` permissions**, including `GetSeed` or `GetRandom`

Randomness uses the SDK-patched `getrandom` 0.2 crate. On hardware it connects
directly to the mandatory `trng-server`, which needs no manifest permission.
Hosted macOS/Linux builds use the host operating system's random source, so they
do not require a simulated TRNG service. Both paths feed 32 bytes into the same
double-SHA-256 conditioning function.

The `security` library is still used for local seed types and SeedQR encoding
and parsing, not for connecting to the security service.
...
SeedQR or typing the words. Importing that copy does not grant access to or
change the device's stored seed.
...
the deprecated `app-config.toml` version field; they cannot build this config.
...
| `src/seed_input.rs` | Checked seed parsing and SDK conversion |
| `src/entropy.rs` | Random source and entropy conditioning |
| `src/app.rs` | App state and callback wiring |

`seed-core` has no KeyOS or Slint dependency, so it builds and tests on the host.
The SDK input and entropy helpers also have host-side integration tests.
...
- **Unsupported 15/18/21-word scans and direct conversions are rejected.**
  Supported text, Standard and Compact inputs round-trip; invalid and mismatched
  inputs do not verify.
