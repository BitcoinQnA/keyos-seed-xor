...
The scheme is [Coldcard's Seed XOR][spec], an open standard that invites other
implementations. This is one.

[spec]: https://github.com/Coldcard/firmware/blob/master/docs/seed-xor.md
...
Coldcard's document describes XOR-ing 11-bit word indices while excluding the
...
Coldcard-compatible tool, which random mode does not, so random is the better
...
- **Both of Coldcard's acceptance vectors**, 24 words and 12 words, three parts
...
exact byte serialisation, and getting it wrong produces parts a real Coldcard
will not reproduce, silently, because they are still valid seeds. It is gated on
reading Coldcard's own source and adding a vector generated by real hardware, not
on inference from the prose.
